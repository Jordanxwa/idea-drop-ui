...
const linkClasses = clsx({
  ...
  "text-center mt-4 inline-block bg-blue-600 text-white px-4 py-2 rounded hove:bg-blue-700 transition": button
})
  ...
  return ( 
    <div className='border border-gray-300 p-4 rounded shadow bg-white flex flex-col justify-between'>
        <div>
          ...
          <Link to='/ideas/$ideasid' params={{ideasid: idea._id.toString()}} className={linkClasses} >
          {button === true ? 'View Idea' : 'Read More'}
          </Link>
        </div>
        </div>
   );
